...
Current repo state: `app.json` already has the Android package (`com.bradyvb.ovalapp`), adaptive icon, and App Links intent filters; `web.ts` already serves `/.well-known/assetlinks.json`; an Android notification channel is registered at HIGH importance; backend push (expo-server-sdk) is platform-agnostic. Remaining gaps:

- [ ] **Package name check.** `com.bradyvb.ovalapp` is permanent once uploaded — it contains your name. If that bothers you, change it to e.g. `com.theovalapp.app` BEFORE the first upload (it's invisible-ish to users but shows in the Play URL: `play.google.com/store/apps/details?id=...`). Update `app.json`, `ANDROID_PACKAGE` on the backend, and assetlinks accordingly.
- [ ] **FCM credentials.** Expo push on Android requires Firebase: create a Firebase project, add the Android app, download `google-services.json` (reference it via `android.googleServicesFile` in app.json), and upload the **FCM V1 service-account key** to EAS credentials (`eas credentials`). No backend changes needed — Expo's push service routes to FCM automatically.
- [ ] **First Android build:** `eas build --platform android --profile production` (produces .aab; EAS manages the upload keystore).
- [ ] **App Links SHA-256.** After Play App Signing is set up (first upload), copy the app-signing cert SHA-256 from Play Console → set the `ANDROID_SHA256` env (backend `.env.example` + `productionEnv.ts` per repo convention) so `assetlinks.json` verifies. Until then Android deep links fall back to the browser interstitials — acceptable, not silent breakage.
