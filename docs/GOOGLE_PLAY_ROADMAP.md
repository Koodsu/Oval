...
Current repo state: `app.json` uses the Android package (`com.theovalapp.app`), includes adaptive icon and App Links intent filters, and the landing deployment proxies `/.well-known/assetlinks.json` to the backend; an Android notification channel is registered at HIGH importance; backend push (expo-server-sdk) is platform-agnostic. Remaining gaps:
