...
function trustedStoreUrl(value: string | undefined, hostname: string): string | undefined {
...
const IOS_APP_STORE_URL = trustedStoreUrl(process.env.IOS_APP_STORE_URL?.trim(), 'apps.apple.com');
const ANDROID_STORE_URL = trustedStoreUrl(process.env.ANDROID_STORE_URL?.trim(), 'play.google.com');
...
router.get('/pod/:podId', (req: Request, res: Response) => {
  ...
  const iosStoreButton = IOS_APP_STORE_URL
    ...
    : '';
  const androidStoreButton = ANDROID_STORE_URL
    ? `<a href="${ANDROID_STORE_URL}" class="store-btn" id="androidBtn">
